                        ...
                        <md-card-content>
                            <form data-vv-scope="Synchronization-Form">
                                <div v-for="(setting,i) in settingService.list" :key="i">
                                    <div v-if="setting.settingTypeName ==='spark_sync_setting'"
                            ...
                            <md-tabs>
                                <md-tab @click="tab='main-settings'" id="tab-main-settings" md-label="Main Settings">
                                    <form data-vv-scope="Main-Form">
                                        <div v-for="(setting,i) in settingService.list" :key="i">
                                            <div v-if="setting.settingTypeName ==='spark_sms_setting'"
                                                 class="md-layout md-gutter">
                                                ...
                                                <div class="md-layout-item  md-xlarge-size-33 md-large-size-33 md-medium-size-33 md-small-size-33">
                                                    <md-field
                                                        ...
                                                        />
                                                        <span class="md-error">{{ errors.first('Main-Formsend_elder_'+setting.id) }}</span>
                                                    </md-field>
                                                </div>
                                ...
                                <md-tab @click="tab='notification-settings'" id="tab-notification-settings"
                                    ...
                                    <div v-for="(smsBody,index) in smsBodiesService.lowBalanceNotifierList"
                                         :key="index">
                                        <sms-body ref="smsBody_notification_ref"
                                                  :sms-variable-default-values="smsVariableDefaultValueService.list"
                                                  :sms-body="smsBody"/>
                                    </div>
                                </md-tab>
                                <md-tab @click="tab='meter-reset-settings'" id="tab-meter-reset-settings"
                                    ...
                                    <div class="md-layout md-gutter md-size-100">
                                        <div class="md-layout-item notice-message-area">
                                            ...
                                            Meter Reset Key is for the customers that want to clear the meter's
                                            error state (throttle error or protect) if one exists. When the
                                            customers want to reset their meter, they need to send this key as SMS"
                                        </div>
                                        <div class="md-layout-item md-size-100">
                                            <md-field :class="{'md-invalid': errors.has('meter_reset_key')}">
                                                ...
                                                <md-input
                                                        ...
                                                        v-validate="'required'"
                                                ></md-input>
                                                <span class="md-error">{{ errors.first('meter_reset_key')}}</span>
                                            </md-field>
                                        </div>
                                        <div class="md-layout-item md-size-100">
    ...
    methods: {
        ...
        async updateSmsSetting () {
            if (this.tab === 'notification-settings') {
                ...
                await this.validateSmsBodies(refs)
                if (!this.smsBodiesService.meterResetFeedbackList.filter((x) => !x.validation).length) {
                    try {
